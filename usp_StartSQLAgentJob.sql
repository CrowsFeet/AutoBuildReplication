...
CREATE PROCEDURE dbo.usp_StartSQLAgentJob
...
------------------------------------------------------------------------------------------------------------------------------------------------------------
--
-- Description :	Step nn - SP that identifies the replication SQL Agent jobs and starts them
--
-- Author :			Andy G
--
-- Created :		21/06/2018
...
INNER JOIN msdb.dbo.sysjobactivity AS sja ON sja.job_id = t1.Job_id
INNER JOIN msdb.dbo.sysjobs AS sj ON sja.job_id = sj.job_id
WHERE sja.run_requested_date IS NULL 
AND sj.name LIKE '%' + @DBName + '%'
AND Completed = 0
